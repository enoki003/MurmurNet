...
class ConfigManager:
    ...
    def __new__(cls, config_path_or_dict: Optional[Union[str, Dict[str, Any]]] = None):
        """シングルトンパターンの実装"""
    ...
    def __init__(self, config_path_or_dict: Optional[Union[str, Dict[str, Any]]] = None):
        ...
        if self._config is None:
            if isinstance(config_path_or_dict, dict):
                ...
                self._config = self._create_config_from_dict(config_path_or_dict)
            else:
                # ファイルパスまたはNoneの場合
                self.config_path = self._find_config_file(config_path_or_dict)
    ...
    def _create_config_from_dict(self, raw_config: Dict[str, Any]) -> MurmurNetConfig:
        ...
        try:
            ...
            logger.info("設定辞書からの設定作成が完了しました")
            return config
            
        except Exception as e:
            logger.error(f"設定辞書処理エラー: {e}")
            raise ConfigValidationError(f"設定辞書の処理に失敗しました: {e}")
    ...
    def rag_mode(self) -> str:
        """RAGモードを取得"""
        return self._config.rag.rag_mode
    
    @property
    def debug(self) -> bool:
        """デバッグフラグを取得"""
        return self._config.logging.debug
    
    @property
    def num_agents(self) -> int:
        """エージェント数を取得"""
        return self._config.agent.num_agents
    
    @property
    def use_parallel(self) -> bool:
        """並列処理フラグを取得"""
        return self._config.agent.use_parallel
